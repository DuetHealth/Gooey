...
struct ConstraintGroupTests {
    ...
    @Test func insetsFromPreviousValues() {
        ...
        #expect(constraintsToArray(group.constraints) == [10, 10, -10, -10])
        group.inset(by: .at(top: 5, left: 5, bottom: 5, right: 5))
        #expect(constraintsToArray(group.constraints) == [15, 15, -15, -15])
    }

    @Test func outsetsFromPreviousValues() {
        let group = mockElement1.goo.boundingAnchor.makeEdges(equalTo: mockElement2)
    ...
    @Test func patchesFromPreviousValues() {
        let group = mockElement1.goo.boundingAnchor.makeEdges(equalTo: mockElement2)
        group.patch(with: .at(top: 10, left: 0, bottom: 10, right: 0))
        #expect(constraintsToArray(group.constraints) == [10, 0, -10, 0])
        group.patch(with: .at(top: 0, left: 5, bottom: 0, right: 5))
        #expect(constraintsToArray(group.constraints) == [10, 5, -10, -5])
    }

    @Test func insetsCorrectValues() {
        let group = mockElement1.goo.boundingAnchor.makeEdges(equalTo: mockElement2, insets: .at(top: 2, left: 4, bottom: 6, right: 7))
        #expect(constraintsToArray(group.constraints) == [2, 4, -6, -7])
        group.inset(by: .at(top: 2, left: 5, bottom: 8, right: 13))
        #expect(constraintsToArray(group.constraints) == [4, 9, -14, -20])
        group.inset(by: .at(top: 3, left: -2, bottom: -3, right: 5))
        #expect(constraintsToArray(group.constraints) == [7, 7, -11, -25])
    }

    @Test func combinedInsetOutset() {
        let group = mockElement1.goo.boundingAnchor.makeEdges(equalTo: mockElement2)
        group.outset(by: .at(top: 10, left: 10, bottom: 10, right: 10))
        #expect(constraintsToArray(group.constraints) == [-10, -10, 10, 10])
        group.inset(by: .at(top: -5, left: -5, bottom: -5, right: -5))
        ...
        #expect(constraintsToArray(group.constraints) == [-17, -13, 17, 13])
    }
